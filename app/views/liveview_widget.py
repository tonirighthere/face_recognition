...
class LiveViewWidget(QWidget):
    def __init__(self):
        ...
        self.worker.recognition_result.connect(self.update_info)
        ...
        if recognized:
            ...
            if db_person:
                ...
                if db_person.get('anh_path'):
                    pix = QPixmap(db_person['anh_path'])
                    if not pix.isNull():
                        self.lbl_avatar.setPixmap(pix.scaled(150, 150, Qt.KeepAspectRatioByExpanding, Qt.SmoothTransformation))
                else:
