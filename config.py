...
#  Pipeline & Hiển thị 
QUEUE_MAXSIZE  = 1          # Kích thước tối đa của các hàng đợi (đảm bảo Real-time)
...
FILTER_YAW_MIN         = 0.35     # Tỷ lệ dist(nose,left_eye)/dist(nose,right_eye) tối thiểu (cho phép quay mặt nhiều hơn)
FILTER_YAW_MAX         = 1.3     # Tỷ lệ trên tối đa (ngoài khoảng → quay mặt → loại)
FILTER_PITCH_THRESHOLD = 1.2     # Độ lệch mũi so với đường mắt (đã normalize theo eye_dist) - nới lỏng để cho phép cúi/ngẩng sâu hơn
